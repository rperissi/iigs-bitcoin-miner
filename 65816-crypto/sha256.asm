...
* NOTE: the round-constant table 'k' formerly lived in its own private segment here.
* It is read via DP-indirect addressing, which resolves through the DATA BANK register.
* The C glue sets DBR to the caller's data bank (bank 0 in the small model), so in the
* small model 'k' (also bank 0) was reachable - but under the LARGE model (occ -b) the
* library's code/data is relocated to another bank and the constants were read from the
* wrong bank, silently corrupting every hash. Fix: 'k' now lives INSIDE the
* SHA256_PROCESSBLOCK segment (see end of file), and PROCESSBLOCK forces DBR to its own
* (== k's) bank for the duration of the block, restoring it before returning. This is
* correct in both memory models. (Local change to upstream 65816-crypto.)
...
SHA256_PROCESSBLOCK start
	...
	phk			; this segment's bank == k's bank (k is in this segment)
	plb			; DBR = our bank so the (zero),y reads of k hit the right bank
	lda	h0
	sta	a_
	lda	h0+2
	sta	a_+2
...
* Round constants, kept in THIS segment (after the rts, so never executed) so that
* phk gives their bank. Read via (zero),y with DBR forced to this bank above.
k	anop
